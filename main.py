...
# --- CONFIGURAÇÕES ---
# Substitua o SEU_ID_AQUI pelo código da sua API no SheetDB
SHEETDB_API_URL = "https://sheetdb.io/api/v1/l8lb0csbymhga"
# Seu WhatsApp com 55 + DDD + Número
SEU_CELULAR = "5511999999999" 
# Senha para você acessar o painel
SENHA_MESTRE = "1234"
...
if menu == "Agendar Aula":
    ...
    if submit:
        if not nome or not whatsapp_aluno or not email_aluno:
            st.warning("⚠️ Por favor, preencha todos os campos (Nome, WhatsApp e E-mail).")
        else:
            ...
            with st.spinner('Verificando agenda...'):
                try:
                    ...
                    if ocupado:
                        ...
                        st.success(f"✅ Sucesso! Agendado para {data_br} às {hora_texto}.")
                        st.balloons()
                        
                        msg = f"Oi! Sou o {nome}. Agendei aula de {estilo} para o dia {data_br} às {hora_texto}."
                    ...
                    st.error("Erro na conexão. Verifique se o SheetDB está configurado.")

# --- TELA 2: PAINEL (PROFESSOR) ---
